...
/-- Name-stable packaging of the nucleus laws for `sieveNucleus` (used by the unified-system plan). -/
theorem sieveNucleus_is_nucleus (J : GrothendieckTopology C) (X : C) :
    (∀ S : Sieve X, sieveNucleus (C := C) J X (sieveNucleus (C := C) J X S) = sieveNucleus (C := C) J X S) ∧
    (∀ S : Sieve X, S ≤ sieveNucleus (C := C) J X S) ∧
    (∀ S T : Sieve X,
      sieveNucleus (C := C) J X (S ⊓ T) = sieveNucleus (C := C) J X S ⊓ sieveNucleus (C := C) J X T) := by
  refine ⟨?_, ?_, ?_⟩
  · intro S
    exact Nucleus.idempotent (n := sieveNucleus (C := C) J X) S
  · intro S
    exact Nucleus.le_apply (n := sieveNucleus (C := C) J X) (x := S)
  · intro S T
    exact Nucleus.map_inf (n := sieveNucleus (C := C) J X) (x := S) (y := T)
